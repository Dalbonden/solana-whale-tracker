...
export async function classifyPosition(
  ...
  side: 'buy' | 'sell',
  tokenAmount: number
): Promise<{ isNewPosition: boolean; isFullExit: boolean }> {
  const history = await getPositionHistory(whale, mint);
  const heldBefore = history.boughtAmount - history.soldAmount;

  if (side === 'buy') {
    // "New" means no meaningful prior balance and no prior buy on record.
    const isNewPosition = history.tradeCount === 0 || heldBefore <= 0;
    return { isNewPosition, isFullExit: false };
  }

  if (heldBefore <= 0) {
    // Selling something we never saw them buy — history predates tracking, so
    // we cannot claim this closed the position.
    return { isNewPosition: false, isFullExit: false };
  }

  const remaining = heldBefore - tokenAmount;
  const isFullExit = remaining / heldBefore <= config.alerts.fullExitResidual;
  return { isNewPosition: false, isFullExit };
}
