...
-- ---------------------------------------------------------------------------
-- wallet_traces — cache for the funding-graph walk (see lib/core/trace-store).
--
-- Holds each wallet's paging cursor so a later visit continues where the last
-- one stopped instead of restarting at the newest transaction. That is what
-- lets repeated analyses reach further back than any single request could.
--
-- Optional: forensics degrades gracefully without it.
-- ---------------------------------------------------------------------------
